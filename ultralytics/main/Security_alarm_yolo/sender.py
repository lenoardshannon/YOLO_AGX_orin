...
# to_email = "181619385662163.com"  # receiver email
#
...
class ObjectDetection:
    ...
    def play_sound(self):
        '''Play an alert sound.'''
        output_warning = "/home/leonardzhang/Yolo_leonard/ultralytics/utils/Warning.mp3"
        playsound(output_warning)
    ...
    def __call__(self):
        ...
        while True:
            ...
            value = im0[:, 4].max().item()  # 解决看到特定帧的报警，每取4个值，找到最大置信读
            _, encoded_img = cv2.imencode('.jpg', im0)
            if value > 0.98:
                filter_count += 1  # 增加一个滤波器 小延时，防止误报 累计到15个数再报告
                if filter_count > 15:
                    filter_count = 0
                if len(class_ids) > 0:  # Only send email If not sent before
                    ...
                    if not self.audio_played:
                        ...
                        audio_thread = threading.Thread(target=self.play_sound)
                        audio_thread.start()
